...
export default function ImageGenNode({ id, data }: NodeProps) {
  ...
  // Live graph for connected image indicator
  ...
  // Check if the required key exists for the selected provider
  const missingKey =
  ...
  }, [id, nodeData, updateNodeData, history, defaultModel]);
  ...
  return (
    ...
    >
      {/* Input handles */}
      <Handle type="target" position={Position.Top} id="prompt" style={{ top: -5, left: "40%" }} title="Prompt input" />
      <Handle type="target" position={Position.Left} id="reference" style={{ left: -5, top: "50%" }} title="Reference image" />
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2a2a2a]">
        ...
        <span className="text-xs font-semibold text-[#378ADD] uppercase tracking-wide">Image Gen</span>
        {nodeData.isRunning && <span className="ml-auto text-[#378ADD]"><SpinnerIcon /></span>}
      </div>
      ...
      <div className="p-3 space-y-2">
        ...
        {/* Model dropdown — Replicate or Gemini */}
        {provider === "replicate" ? (
          <select
            value={(nodeData.model as string) || defaultModel}
        ...
        {/* Connected images indicator */}
        {connectedImages.length > 0 && (
          <div className="flex items-center gap-1.5 px-2 py-1 bg-[#378ADD15] border border-[#378ADD30] rounded-lg">
        ...
        <button
          ...
          style={{ background: provider === "gemini" ? "#4285f4" : "#378ADD" }}
        >
